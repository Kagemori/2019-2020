/* fire_4.c - step 4
 * introduce outer loop, probabilities
 */
#include <stdio.h>
#include <stdlib.h>
#include "X-graph.h"
...
int main(int argc, char ** argv) {
    ...
    buffer = (double *) malloc (n_probs*sizeof(double));
    ...
    MPI_Reduce(percent_burned,buffer,n_probs,MPI_DOUBLE,MPI_SUM,0,
        MPI_COMM_WORLD);

    for (i_prob=0;i_prob<n_probs;i_prob++)
        percent_burned[i_prob]=buffer[i_prob];

    // plot graph
    if (do_display==1 && rank==0) {
